...
export function VehicleCard({
  ...
  return (
    <article className="group relative flex flex-col bg-background border border-transparent hover:border-border transition-all duration-300">
      {/* Image — links to detail page */}
      <Link href={{ pathname: '/fahrzeuge/[slug]', params: { slug: vehicle.slug } }} className="block">
        ...
        <div className="flex flex-col gap-3 p-5 pb-4">
          ...
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 font-sans text-xs text-muted">
            {vehicle.mileage_km != null && (
              <span>{formatMileage(vehicle.mileage_km, locale === 'de' ? 'de-DE' : 'en-GB')}</span>
            )}
            {vehicle.fuel_type && (
              <>
      ...
      <div className="px-5 pb-5">
        <Link
          href={{ pathname: '/fahrzeuge/[slug]', params: { slug: vehicle.slug }, hash: 'anfragen' } as never}
          className="flex w-full items-center justify-center gap-2 border border-gold/40 bg-gold/5 py-2.5 font-sans text-xs text-gold transition-colors hover:bg-gold/10"
        >
          {inquireLabel}
